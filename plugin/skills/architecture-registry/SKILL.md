...
To ensure non-negotiable architectural compliance and keep your main context light, this plugin bundles two pre-configured, native subagents inside its `agents/` (for AG2.0) and `.opencode/agents/` (for opencode) directories:
...
1. **`ComponentPlanner`**
   ...
   * **Permissions**: Access to MCP registry tools enabled; write access to code sandbox files disabled.
...
2. **`ComponentImplementer`**
   * **Role**: Sandboxed coder. Executes the planned `modification_tasks` itemized in the component's registry. Implements code first via Test-Driven Development (TDD), marks tasks as completed, and executes local validation commands.
   * **Permissions**: Access to both MCP registry tools and full sandboxed filesystem/terminal capabilities.
...
#### Choreography & Execution Loop (Multi-Harness Support):
* **Task Spawning**: The primary orchestrator agent MUST never plan or implement code directly. Instead, delegate the work by spawning the appropriate specialized subagent using whatever active subagent execution tool is present in your current environment:
  - **In opencode**: Use the native `task` tool passing the Component ID and instructions in the `prompt`, the specialized agent type (`"ComponentPlanner"` or `"ComponentImplementer"`) in the `subagent_type` field, and a short `description`.
  - **In Antigravity 2.0 (AG2.0)**: Use the `invoke_subagent` tool passing the target Component ID and instructions in the `prompt`, and the unique agent name in the `TypeName` parameter.
* **Direct Contract Retrieval**: Once spawned, the subagent will immediately run the `compile_component_contract` tool to fetch its authoritative, fully resolved specification and tasks in Markdown, completely avoiding main context bloat.
